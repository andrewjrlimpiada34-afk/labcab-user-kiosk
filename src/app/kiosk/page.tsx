...
export default function KioskPage() {
  ...
  const apparatusQuery = useMemo(() => {
    if (!db) return null;
    return query(collection(db, 'apparatus'));
  }, [db]);

  const { data: apparatus, loading: inventoryLoading } = useCollection<InventoryItem>(apparatusQuery);

  const filteredInventory = useMemo(() => {
    const list = apparatus ?? [];
    if (!search.trim()) return list;
    const s = search.toLowerCase();
    return list.filter((item) => (item.name || '').toLowerCase().includes(s));
  }, [apparatus, search]);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="bg-white border-b p-6 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">LabCab Kiosk</h1>
            <Badge variant="outline" className="text-xs uppercase tracking-wider">Kiosk</Badge>
          </div>
        </div>
          ...
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
            {['All', 'Glassware', 'Heating', 'Tools'].map((cat) => (
              <Button
                ...
                onClick={() => {
                  // category UI is currently cosmetic; keep behavior simple
                  if (cat === 'All') setSearch('');
                  // If you later want real category filtering, connect it to Firestore fields.
                  if (cat !== 'All') setSearch(cat);
                }}
              >
                {cat}
              </Button>
